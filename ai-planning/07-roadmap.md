...
- `data/passages.json` schema established; hard-coded bunny passage migrated into it with precomputed integer tier counts (t1=43 t2=35 t3=39 t4=15).
- `src/corpus/passages.ts` imports `passages.json`; derives `Composition` and `difficulty` at load (not stored in JSON — thresholds stay volatile until 1f).
- Difficulty bin shown on results screen. Difficulty filter (All / Easy / Medium / Hard) on test screen; options with zero passages are disabled.
- `scripts/build-passages-json.ts` added for Phase 1f passage onboarding.
- frequency.json NOT imported into the browser bundle (no Phase 1e consumer; defer to Phase 2 when per-word tier lookup is needed).
